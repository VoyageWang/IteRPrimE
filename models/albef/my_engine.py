...
class ALBEF(VLPModel):
    ...
    def get_gradcam(
        ...
        output = self.model(image_tensor, text_input)
        loss = output[:, 1].sum()
        ...
        main_words_ids, id_map = self.find_main_words(query, start_idx, self.tokenizer)
        all_focus_ids = []
