...
def collate_fn(batch):
    ...
    batch[0] = nested_tensor_from_tensor_list(batch[0], size_divisibility=1) 
...
def nested_tensor_from_videos_list(videos_list: List[Tensor], size_divisibility=1):
    ...
    padded_videos = torch.zeros(padded_batch_shape, dtype=dtype, device=device) # 创建一个新的tensor用于存储pad后的video
    videos_pad_masks = torch.ones((b, t, h, w), dtype=torch.bool, device=device) # 创建一个新的tensor用于区别pad后的图片哪些是原始部分，哪些是padding部分
