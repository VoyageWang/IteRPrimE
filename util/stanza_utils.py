...
nlp = stanza.Pipeline('en', dir = "../checkpoints/pweight", download_method=None)

MAIN_POS = ['NOUN', 'VERB', 'ADJ', 'PROPN', 'NUM']

def find_no_main_words(sent, start_idx, tokenizer):
    # id_map = {}
    # doc = nlp(sent)
    # cursor = start_idx + 1
    # main_words_ids = []
    # for word in doc.sentences[0].words:
    #     temp = cursor; cursor += 1
    ...
    cnt = 0; rword = ""; flag = False
    for word in doc.sentences[0].words:
        if word.text != sent_split[cnt]:
            if word.pos: flag = True
            rword += word.text
            if rword != sent_split[cnt]:
                continue
            else:
                word.text = rword
                rword = ""
                if flag: word.pos = MAIN_POS[0]
                flag = False                
        cnt += 1
        temp = cursor
...
def find_main_words(sent, start_idx, tokenizer):
    ...
    cnt = 0; rword = ""; flag = False
    for word in doc.sentences[0].words:
        if word.text != sent_split[cnt]: # 如果当前单词从中间被分解了
            if word.pos in MAIN_POS: flag = True
            rword += word.text
            if rword != sent_split[cnt]:
                continue
            else:
...
def find_agent_by_stanza(sent):
    ...
    for word in doc.sentences[0].words:
        if agent == word.text: 
            agent_id = word.id
    return agent, agent_id, len(doc.sentences[0].words)

if __name__ == '__main__':
    text = "a red and white checkered table with two wooden chairs"
    print(find_agent_by_stanza(text)[0])
